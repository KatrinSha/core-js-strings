...
function isString(value) {
  const temp = value;
  return typeof temp === 'string' || temp instanceof String;
}
...
function formatTime(minutes, seconds) {
  ...
  min = min.length > 1 ? min : min.padStart(2, 0);
  sec = sec.length > 1 ? sec : sec.padStart(2, 0);
...
function countVowels(str) {
  ...
  let count = 0;
  for (let i = 0; i < str.length; i++) {
    if (vowels.includes(str[i]) || vowels.toUpperCase().includes(str[i])) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  str = str.toLowerCase().replace(regxp, '');
